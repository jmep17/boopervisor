...
- [x] Hook entries are added, edited and removed per event, with events drawn from the catalog rather than typed by hand.
- [x] A hook missing a required field is refused before the write.
- [x] The script a hook points at is shown read-only, with its path, and is never rewritten by Boopervisor.
- [x] The per-scope breakdown shows which scope contributed which hooks.
